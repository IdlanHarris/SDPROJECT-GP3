...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['remove_id'])) {
  ...
  error_log("Received remove_id: " . $userId);
...
$query = "SELECT user_id, username, email, phone_number, membership FROM users WHERE user_type = 'member'";
...
<div class="container">
    ...
    <tbody id="memberTableBody">
      ...
      if ($result && $result->rowCount() > 0) {
          while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
              ...
              echo "<td class='membership'>" . htmlspecialchars($row['membership']) . "</td>";
  ...
  <div class="mt-2"><a href="adminDashboard.php#section3"><button type="submit" class="btn btn-primary">↩</button></a></div>
...
$(document).on('click', '.remove-btn', function() {
    ...
    console.log("Attempting to delete user_id:", userId); // Debugging log
    ...
    if (confirm('Are you sure you want to remove this member?')) {
        ...
        $.ajax({
            type: 'POST',
            url: 'manageMember.php', // Server-side script for member removal
            ...
            error: function(jqXHR, textStatus, errorThrown) {
                console.error('AJAX Error:', textStatus, errorThrown); // Enhanced error handling
